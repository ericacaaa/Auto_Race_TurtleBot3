...
from detect.cfg import DetectTrafficLightParamsConfig
...
class DetectTrafficLight():
    def __init__(self):
        ...
        self.hue_yellow_l = rospy.get_param("~hue_yellow_l", 20)
        self.hue_yellow_h = rospy.get_param("~hue_yellow_h", 35)
        self.saturation_yellow_l = rospy.get_param("~saturation_yellow_l", 100)
        self.saturation_yellow_h = rospy.get_param("~saturation_yellow_h", 255)
        self.lightness_yellow_l = rospy.get_param("~lightness_yellow_l", 50)
        self.lightness_yellow_h = rospy.get_param("~lightness_yellow_h", 255)

        self.hue_green_l = rospy.get_param("~hue_green_l", 46)
        self.hue_green_h = rospy.get_param("~hue_green_h", 76)
        self.saturation_green_l = rospy.get_param("~saturation_green_l", 86)
        self.saturation_green_h = rospy.get_param("~saturation_green_h", 255)
        self.lightness_green_l = rospy.get_param("~lightness_green_l", 50)
        self.lightness_green_h = rospy.get_param("~lightness_green_h", 255)
        ...
        if self.calibration_mode == True:
            ...
            self.pub_image_red_light = rospy.Publisher('/detect/traffic_light/red/compressed', CompressedImage, queue_size = 1)
            self.pub_image_yellow_light = rospy.Publisher('/detect/traffic_light/yellow/compressed', CompressedImage, queue_size = 1)
            self.pub_image_green_light = rospy.Publisher('/detect/traffic_light/green/compressed', CompressedImage, queue_size = 1)

        # Subscriber
        self.sub_img = rospy.Subscriber('/image_input', CompressedImage, self.cbGetImage, queue_size = 1)

        # Publisher
        self.pub_img = rospy.Publisher('/detect/traffic_light/compressed', CompressedImage, queue_size = 1)
    ...
    def fnFindTrafficLight(self):
        ...
        if self.green_count >= 5:
            ...
            # go to next mission
            self.pub_mission.publish(self.Mission.Intersection.value)
        ...
        self.pub_img.publish(self.cvBridge.cv2_to_compressed_imgmsg(self.cv_image, "jpg"))
